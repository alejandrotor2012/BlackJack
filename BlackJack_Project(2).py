...
class Player:
    ...
    def __init__(self,name='',funds=0, wager =0, action=''):
...
while play == True:
    ...
    while True:
        ...
        if ((hand_converter(player1.hand) > hand_converter(house.hand)) and (hand_converter(player1.hand) <= 21)) or (hand_converter(house.hand) > 21):
            winner()
            play = keep_playing(play)
            break
